...
class Slider : public ScrollableFrame {
    public:
        Slider(Rectangle bounds_, string text_left_, string text_right_, float min_, float max_, float init, function<void(float num)> callback_);
    
        void render(Vector2 scrollOffset) override;

    private:
        float value, last_value, min, max;
